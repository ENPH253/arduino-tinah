...
#define lcd_running_app() do { \
  lcd_delay();\
  hd44780_send_command(HD44780_SET_CURSOR_COMMAND | 0x40);\
  lcd_delay();\
  lcd_P(PSTR("Running App     "));\
} while(false) 
...
//Initialization as a local static variable not working, so this is initialized
//in main.
static bool wait_forever;
void initial_wait(void)
{
  const uint32_t progress_bar_delay_max = F_CPU / 128;
  const uint8_t progress_bar_max = 16;
  uint32_t progress_bar_delay = 0;
  ...
  while (!boot_serial_available())
  {
    ...
    if (!wait_forever) {
      if (progress_bar_delay >= progress_bar_delay_max) {
        progress_bar_delay = 0;
        if (progress_bar >= progress_bar_max)
        {
          lcd_running_app();
          _delay_ms(500);
          app_start();
...
int main(void)
{
  ...
  debugstr_P(PSTR("Serial Bootloader starting\r"));

	hd44780_init(HD44780_MODE_2_LINE | HD44780_MODE_5X8);
	hd44780_send_command(HD44780_DISP_CTRL_COMMAND | HD44780_DISP_CTRL_DISPLAY_ON);
  lcd_delay();
	hd44780_send_command(HD44780_SET_CURSOR_COMMAND);
  lcd_delay();
  lcd_P(PSTR("TINAHBoot 0.1.0 "));
	hd44780_send_command(HD44780_SET_CURSOR_COMMAND | 0x40);
  lcd_delay();
